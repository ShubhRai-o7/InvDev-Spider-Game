Shader "Lpk/LightModel/ToonLightBase"
{
    Properties
    {
        ...
        [Space]   
        _OutlineWidth      ("OutlineWidth", Range(0.0, 3))      = 0.15
        ...
        Pass
    {
        ...
        v2f vert(appdata v)
        {
            ...
            // Adjust the outline width scaling factor based on the orthographic size and outline width
            float outlineFactor = _OutlineWidth * (_OrthoSize * 0.01); // Ensure OutlineWidth affects the scaling

            VertexPositionInputs vertexInput = GetVertexPositionInputs(v.vertex.xyz);

            // Offset the vertex along its normal direction by the adjusted outline width
            o.pos = TransformObjectToHClip(float4(v.vertex.xyz + v.normal * outlineFactor, 1));
